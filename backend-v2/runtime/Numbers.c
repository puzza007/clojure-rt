...
static Ratio *toRatio(RTValue v) {
  ...
  if (t == ratioType)
    return (Ratio *)RT_unboxPtr(v);
  if (t == integerType)
    return Ratio_createFromInt(RT_unboxInt32(v));
  if (t == bigIntegerType)
    return Ratio_createFromBigInteger((BigInteger *)RT_unboxPtr(v));
...
static BigInteger *toBigInt(RTValue v) {
  objectType t = getType(v);
  if (t == bigIntegerType)
    return (BigInteger *)RT_unboxPtr(v);
  if (t == integerType)
    return BigInteger_createFromInt(RT_unboxInt32(v));
  return NULL;
}
...
double Numbers_toDouble(RTValue v) {
  objectType t = getType(v);
  if (t == doubleType)
    return RT_unboxDouble(v);
  if (t == integerType)
    return (double)RT_unboxInt32(v);
  if (t == bigIntegerType)
    return BigInteger_toDouble((BigInteger *)RT_unboxPtr(v));
  if (t == ratioType)
    return Ratio_toDouble((Ratio *)RT_unboxPtr(v));
...
static bool isNumeric(objectType t) {
  return t == integerType || t == doubleType || t == bigIntegerType ||
         t == ratioType;
}
  ...
  RTValue Numbers_##NAME(RTValue a, RTValue b) {                               \
    ...
    if (!isNumeric(ta) || !isNumeric(tb)) {                                    \
      ...
      throwIllegalArgumentException_C(                                         \
          "Arithmetic operation requires numeric arguments");                  \
    }                                                                          \
...
#define GENERIC_CMP_OP(NAME, OP_BIGINT, OP_RATIO, OP_DOUBLE)                   \
  bool Numbers_##NAME(RTValue a, RTValue b) {                                  \
    ...
    if (!isNumeric(ta) || !isNumeric(tb)) {                                    \
      ...
      release(b);                                                              \
      throwIllegalArgumentException_C(                                         \
          "Comparison requires numeric arguments");                            \
    }                                                                          \
    if (ta == doubleType || tb == doubleType) {                                \
...
bool Numbers_equiv(RTValue a, RTValue b) {
  ...
  if (ta == tb) {
    if (ta == integerType)
      return RT_unboxInt32(a) == RT_unboxInt32(b);
    if (ta == doubleType)
      return RT_unboxDouble(a) == RT_unboxDouble(b);
  ...
  // For now, let's use the same logic as comparisons but specifically for
  // equality
  if (ta == doubleType || tb == doubleType) {
...
#define BINARY_MATH_WRAPPER(NAME, FUNC)                                        \
  RTValue Numbers_##NAME(RTValue a, RTValue b) {                               \
    if (!isNumeric(getType(a)) || !isNumeric(getType(b))) {                    \
      ...
      release(b);                                                              \
      throwIllegalArgumentException_C(                                         \
          "Math operation requires numeric arguments");                        \
    }                                                                          \
    double va = Numbers_toDouble(a);                                           \
    double vb = Numbers_toDouble(b);                                           \
...
#define M_LN10_X 2.30258509299404568402
  double va = Numbers_toDouble(a);
